class InitializeAutofill {
    ...
    for (const item of fields) {
      ...
      switch (item.function) {
        ...
        case 'externalFunction':
          await this.evalFunction(item.functionName)
  ...
  consultBorrower() {
    if (this.parent.gbl.avs()) return false;
    this.parent.gbl.seta_avs(50);
    this.doc.frm.hdd_in_substituto_trib_tom.value = "0";
    this.doc.frm.hdd_in_retido_not.value = "0";
    this.parent.gbl.acessa('g', "snfse00100a3.asp", "txt_cnpj_cpf_tom="+this.doc.frm.txt_cnpj_cpf_tom.value);
  }

  async evalFunction(functionName){
    await new Promise(resolve => {

      this.frame[functionName]()
